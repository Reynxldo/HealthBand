...
uint32_t inicioTempBaja = 0;
bool bpmAltoActivo      = false;
bool spo2BajoActivo     = false;
bool tempAltaActiva     = false;
bool tempBajaActiva     = false;

String alertaPendiente = "";
volatile bool latidoDetectado = false;
...
#define BPM_MAXIMO   100.0
#define TEMP_MAXIMA   37.5
#define TEMP_MINIMA   34.8
...
void verificarAnomaliaTemp(float temp) {
  if (temp > TEMP_MAXIMA) {
    if (!tempAltaActiva) {
      ...
      alertaPendiente = "Temperatura elevada";
      alertaBuzzer();
    ...
    inicioTempBaja = 0;
  } else if (temp >= 20.0 && temp < TEMP_MINIMA) {
    if (!tempBajaActiva) {
      tempBajaActiva = true;
      inicioTempBaja = millis();
    } else if (millis() - inicioTempBaja >= TIEMPO_PERSISTENCIA) {
      Serial.println("  → Hipotermia ⚠");
      alertaPendiente = "Hipotermia";
      alertaBuzzer();
      tempBajaActiva = false;
      inicioTempBaja = 0;
    }
...
void loop() {
  ...
  if (millis() - lastTempRead > 2000) {
    ...
    if (temp < 20.0) {
      ...
      tempAltaActiva = false;
      tempBajaActiva = false;
    } else if (temp >= TEMP_MINIMA && temp <= TEMP_MAXIMA) {
      Serial.println("  → Normal");
      tempAltaActiva = false;
      tempBajaActiva = false;
    } else {
  ...
  if (millis() - lastMqttPublish > 10000) {
    ...
    if (alertaPendiente != "") {
      client.publish("pulsera/alerta", alertaPendiente.c_str());
      alertaPendiente = "";
    }
